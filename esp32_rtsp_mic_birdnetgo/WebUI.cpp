...
static String htmlIndex() {
    ...
    h += F(
        ...
        "<div class='page'>"
        "<div class='card'><div class='hero'><div><div class='brand'><div class='title' id='t_title'>ESP32 RTSP Mic for BirdNET-Go</div><span class='badge' id='fwv'></span></div><div class='subtitle'><span id='t_rtsp_label'>RTSP:</span> <a id='rtsp_ip' class='mono' href='rtsp://");
    h += ip;
    h += F(
    ...
    h += F(
        ...
        "<tr><td class='k'><span id='t_time_sync_en'>Time Sync</span><span class='help' id='h_time_sync'>?</span></td><td class='v'><div class='field'><select id='sel_time_sync'><option value='on'>ON</option><option value='off'>OFF</option></select><button onclick=\"setv('time_sync',sel_time_sync.value)\">Set</button></div></td></tr>"
        ...
        "<tr><td class='k' id='t_mdns'>mDNS</td><td class='v'><div class='field'><select id='sel_mdns'><option value='on'>ON</option><option value='off'>OFF</option></select><button onclick=\"setv('mdns_enable',sel_mdns.value)\">Set</button></div></td></tr>"
        ...
        "<button onclick=\"confirmNetReset()\" id='b_net_reset'>Reset Wi-Fi</button>"
        "</div></div>"
...
static inline bool argToInt(const String &name, int32_t &out) { if (!web.hasArg("value")) return false; out = (int32_t) web.arg("value").toInt(); return true; }
...
static void httpSet() {
    ...
    else if (key == "time_offset") { int32_t v; if (argToInt("value", v) && v>=-720 && v<=840) { timeOffsetMinutes = v; configTime(timeOffsetMinutes * 60, 0, NTP_SERVER_1, NTP_SERVER_2); saveAudioSettings(); } }
    else if (key == "mdns_enable") { String v=web.arg("value"); if (v=="on"||v=="off") { mdnsEnabled = (v=="on"); applyMdnsSetting(); saveAudioSettings(); } }
    apiSendJSON(F("{\"ok\":true}"));
}
